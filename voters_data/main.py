...
def create_tables(conn, cur):
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS candidate_name (
            candidate_id VARCHAR(255) PRIMARY KEY,
            candidate_name VARCHAR(255),
            party_affiliation VARCHAR(255),
            biography TEXT,
...
def generate_candidate_data(candidate_number, total_parties):
    ...
    response = requests.get(BASE_URL + '&gender=' + ('female' if candidate_number % 2  == 0 else 'male'))
    ...
    if response.status_code == 200:
        ...
        return {
             ...
             "biography": 'A bried biography of the candidate',
...
def deliver_report(err, msg):
    if err is not None: 
        print(f"Message deivery failed: {err} ")
    else:
        print(f"Message deivered to {msg.topic()} [{msg.partition()}]")    
